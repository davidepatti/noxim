...
bool  TGlobalParams::with_ack                         = DEFAULT_WITH_ACK;  
...
void showHelp(char selfname[])
{
    ...
    cout << "\t-sim N\t\tRun for the specified simulation time [cycles] (default " << DEFAULT_SIMULATION_TIME << ")" << endl;
    cout << "\t-with-ack N\t Send an ack once received a packet (0=disabled, 1=enabled)" << endl<< endl;
    ...
    cout << "And finally if you think the ack packet is useless, please feel free to implement something useful! :-)"<< endl;
    exit(0);
}
...
int sc_main(int arg_num, char* arg_vet[])
{
    ...
    else
    {
        ...
        do
        {
            ...
            }else if(!strcmp(arg_vet[i],"-with-ack")){
                int ack_en=atoi(arg_vet[i+1]);
                if(ack_en==1){
                    TGlobalParams::with_ack=true;
                    i+=2;
                }else if (ack_en==0){
                    TGlobalParams::with_ack=false;
                    i+=2;
                }else{
                    badArgument(arg_vet[i+1], arg_vet[i]);
                }
            }  
