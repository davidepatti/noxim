  ...
  else
  {
    ...
    for(int i=0; i<DIRECTIONS+1; i++)
    {
      ...
      if ( (req_rx[i].read()==1-current_level_rx[i]) && buffer[i].IsFull() ) {
        ...
        {
          cout << sc_time_stamp().to_double()/1000 << ": Router[" << local_id <<"], Input[" << i << "], WARNING, BUFFER FULL!"<< endl;
        }
      }
      ...
      if ( (req_rx[i].read()==1-current_level_rx[i]) && !buffer[i].IsFull() )
      {
        ...
        // If direction is LOCAL then update sentFlit statistics
        if (i == DIRECTION_LOCAL) {
          if(TGlobalParams::verbose_mode > VERBOSE_OFF) 
          {
            cout << sc_time_stamp().to_double()/1000 << ": Router[" << local_id <<"], Input[" << i << "], UPDATE SENT STATS: flit: " << received_flit << endl;
          }
  ...
  else
  {
    ...
    for(int j=0; j<DIRECTIONS+1; j++)
    {
      ...
      if ( !buffer[i].IsEmpty() )
      {
        ...
        if (flit.flit_type==FLIT_TYPE_HEAD || flit.flit_type==FLIT_TYPE_ACK) 
        {
      ...
      if ( !buffer[i].IsEmpty() )
      {
        ...
        if (o != NOT_RESERVED)
        {
          if ( current_level_tx[o] == ack_tx[o].read() )
          {
            ...
            if (flit.flit_type == FLIT_TYPE_TAIL || flit.flit_type == FLIT_TYPE_ACK) {
              if(TGlobalParams::verbose_mode > VERBOSE_OFF)
              {
                cout << sc_time_stamp().to_double()/1000 
                  << ": Router[" << local_id 
                  << "], Input[" << i << "] Release Reservation " << o << " flit: " << flit << endl;
              }
            ...
            if (o == DIRECTION_LOCAL)
            {
              if(TGlobalParams::verbose_mode > VERBOSE_OFF) 
              {
                cout << sc_time_stamp().to_double()/1000 
                  << ": Router[" << local_id 
                  << "], Input[" << i << "] UPDATE RECV STATS: flit: " << flit << endl;
              }
