...
{
  char *apsra_fname;
  char *cg_fname, *rt_fname;

  if (!ParseCommandLine(argc, argv, &apsra_fname, &cg_fname, &rt_fname))
    {
      cout << "Use " << argv[0] << " <apsra fname> <cg fname> <rt fname>" << endl
	   << "\t<apsra fname> output file generated by APSRA" << endl
	   << "\t<cg fname>    communication graph file name" << endl
	   << "\t<rt fname>    routing tables file name" << endl
	   << endl;
