...
void Hub::OpenFile()
	{
		...
		fp.open(ResultsFileName.c_str(),std::ofstream::out);
	...
	void Hub::PrintInFile(Flit flit,int channel)
	{
		//fp<<sc_time_stamp()<<" SRC ID:"<< flit.src_id << " Dest ID:"<<flit.dst_id<<" flit Type : "<<flit.flit_type<<" flit size : "<<flit.sequence_no<<" using channel "<<flit.vc_id<<endl;
		 /*if (flit.vc_id == local_id) 
		 {
			 fp<<sc_time_stamp ()<<"vc_id = local_id) = true "<<endl;
		 }*/
        fp<<endl;
        if (flit.flit_type == 0)
	{
		fp<<sc_time_stamp()<<" SRC ID:"<< flit.src_id << " Dest ID:"<<flit.dst_id<<" HEADER seq length : " <<flit.sequence_length<<" seq num : "<<flit.sequence_no<<" using channel "<<channel<<endl;
	}
       
	if (flit.flit_type == 1)
	{
		fp<<sc_time_stamp()<<" SRC ID:"<< flit.src_id << " Dest ID:"<<flit.dst_id<<" BODY   seq length : " <<flit.sequence_length<<" seq num : "<<flit.sequence_no<<" using channel "<<channel;
	}
       
	if (flit.flit_type == 2)
	{       fp<<endl;
		fp<<sc_time_stamp()<<" SRC ID:"<< flit.src_id << " Dest ID:"<<flit.dst_id<<" TAIL   seq length : " <<flit.sequence_length<<" seq num : "<<flit.sequence_no<<" using channel "<<channel<<endl;
	}
