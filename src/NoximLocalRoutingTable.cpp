...
NoximAdmissibleOutputs NoximLocalRoutingTable::getAdmissibleOutputs(const NoximLinkId& in_link,
							    const int      destination_id)
{
  return rt_node[in_link][destination_id];
}

NoximAdmissibleOutputs NoximLocalRoutingTable::getAdmissibleOutputs(const int in_direction,
							    const int destination_id)
{
