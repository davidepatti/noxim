...
#include "Initiator.h"
   ...
   void Initiator::thread_process()
  {
	  ...
	  int destHub = tile2Hub(flit_payload.intr_id);
	  //cout << " *** Init : Starting transmission of " << flit_payload << " to reach HUB_" << destHub <<  endl;
	  ...
	  if (!trans->is_response_error() )
	  {
	      ...
	      if (flit_payload.flit_type == FLIT_TYPE_TAIL)
	      {
		  //cout << "*** [Ch"<< _channel_id <<"] tail flit sent " << flit_payload << ", releasing token" << endl;
		  hub->flag->write(RELEASE_CHANNEL);
		  // TODO: vector for multiple channel
		  hub->transmission_in_progress[_channel_id] = false;
	      }
	  }
